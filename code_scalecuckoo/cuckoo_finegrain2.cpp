// cuckoo + tags (used only for inserts - not in lookups) + bucket_lock 
...
#define HASH(...) hashlittle(__VA_ARGS__) 
...
static inline int next_bucket(const size_t index, unsigned char tag) {

    // 0x5bd1e995 is the hash constant from MurmurHash2

    return (index ^ ((tag & tagmask ) * 0x5bd1e995)) & bucketmask;

    //return ((index ^ (HASH(&tag,1,0))) & bucketmask);
}
...
bool add_attempt(hashtable_t *ht,kv_obj_t *kv,int bucket, unsigned char tag){
	...
	for(int i =0 ; i<SLOTS_PER_BUCKET; i++){
		...
		if (CHECK_EMPTY_SLOT(ht->table[bucket].slots[i])){
			...
			pair.bucket = bucket;
			pair.slot = i;
			add_kv_in_table(ht,&pair,kv,tag);
...
bool search_phase(hashtable_t *ht, cuckoo_path_t *cuckoo_path,
							 int current_bucket, int victim_slot){
	...
	while((cuckoo_path->path_index) != MAX_CUCKOO_PATH_LEN){
		...
		current_bucket = next_bucket(current_bucket,
					ht->table[current_bucket].slots[victim_slot].tag);
...
void add_kv_in_table(hashtable_t *ht, swap_pair_t *pair, 
	...
	// int version_array_index = lock_hash(ht, pair->bucket, 
	// 								next_bucket(pair->bucket, tag)); 
	...
	pthread_mutex_lock(&ht->table[pair->bucket].lock);

	ht->table[pair->bucket].slots[pair->slot].ptr = kv;
	ht->table[pair->bucket].slots[pair->slot].tag = tag;

	pthread_mutex_unlock(&ht->table[pair->bucket].lock);
	//__sync_fetch_and_add(&ht->version_counters[version_array_index],1);
...
void reverse_swap(hashtable_t *ht, cuckoo_path_t *cuckoo_path){
	...
	while(i>0){
		...
		// 	next_bucket(cuckoo_path->path[i-1].bucket, 
		// 		ht->table[cuckoo_path->path[i-1].bucket].slots[cuckoo_path->path[i-1].slot].tag)); 

		

	
		//__sync_fetch_and_add(&ht->version_counters[version_array_index],1);
		pthread_mutex_lock(&ht->table[cuckoo_path->path[i-1].bucket].lock);
		...
		pthread_mutex_unlock(&ht->table[cuckoo_path->path[i-1].bucket].lock);
...
bool insert(hashtable_t *ht, kv_obj_t *kv){
	...
	//hash = HASH(kv->key.c_str(), kv->key.length(), hash);
	hashlittle2(kv->key.c_str(), kv->key.length(), &hash, &hash2);
	...
	dbg_printf(" Key : %s, hash : %xu, tag : %u , bucket1 : %d, bucket2 : %d \n",
									kv->key.c_str(),hash,tag,bucket1,bucket2);

	
	if(add_attempt(ht,kv,bucket1, tag)){
	...
	if(add_attempt(ht,kv,bucket2, tag)){
		pthread_mutex_unlock(&ht->writer_mutex);
		return true;
	}
	...
	if (search_phase(ht, &cuckoo_path, victim_bucket, victim_slot)){
		...
		add_kv_in_table(ht,&(cuckoo_path.path[0]), kv, tag);
...
kv_obj_t *lookup(hashtable_t *ht, string key){
	...
	//hash = HASH(key.c_str(), key.length(), hash);
	hashlittle2(key.c_str(), key.length(), &hash, &hash2);

	int bucket1 = hash & bucketmask;

	unsigned char tag = tag_hash(hash2);

	int bucket2 = next_bucket(bucket1,tag);

	int initial_counter;
	int version_array_index = lock_hash(ht, bucket1, next_bucket(bucket1, tag)); 

	dbg_printf(" Key : %s, hash : %xu, tag : %u , bucket1 : %d \n",
									key.c_str(),hash,tag,bucket1);
	...
	//do {
		...
		//while((initial_counter = ht->version_counters[version_array_index]) 
		//																%2 != 0);

		if (bucket1 < bucket2){
			pthread_mutex_lock(&ht->table[bucket1].lock);
			pthread_mutex_lock(&ht->table[bucket2].lock);
		}else{
			pthread_mutex_lock(&ht->table[bucket2].lock);
			pthread_mutex_lock(&ht->table[bucket1].lock);
		}
		...
		for(int i=0; i<SLOTS_PER_BUCKET; i++){
			...
			if(ht->table[bucket1].slots[i].ptr){

				dbg_printf("tag matched for bucket %d slot %d \n",bucket1,i);
				if(check_key(ht->table[bucket1].slots[i].ptr->key,key)){

					ans_kv = ht->table[bucket1].slots[i].ptr;
					
					// //dbg_printf("version counter : %d \n", ht->version_counters[version_array_index]);
					// if(initial_counter == ht->version_counters[version_array_index]){
					// 	return ans_kv;
					// }

					pthread_mutex_unlock(&ht->table[bucket1].lock);
					pthread_mutex_unlock(&ht->table[bucket2].lock);
		...
		for(int i=0; i<SLOTS_PER_BUCKET; i++){
			...
			if(ht->table[bucket2].slots[i].ptr){

				if(check_key(ht->table[bucket2].slots[i].ptr->key,key)){

					ans_kv = ht->table[bucket2].slots[i].ptr;
					// if(initial_counter == ht->version_counters[version_array_index]){
					// 	return ans_kv;
					// }
