// Cuckoo + tags (both) + version_count 
...
bool insert_cuckoo_path(hashtable_t *ht,cuckoo_path_t *cuckoo_path, int bucket, int slot){
	...
	//cuckoo_path->path[cuckoo_path->path_index].slot_val = ht->table[bucket].slots[slot];
...
kv_obj_t *lookup(hashtable_t *ht, string key){
	...
	int initial_counter;
	int version_array_index = lock_hash(ht, bucket, next_bucket(bucket, tag)); 

	dbg_printf(" Key : %s, hash : %xu, tag : %u , bucket1 : %d \n",
									key.c_str(),hash,tag,bucket);
	...
	do {
		...
		//if(!key.compare("0"))
		//	cout << "bucket1 for 0: " << bucket << next_bucket(bucket,tag) << endl;

		for(int i=0; i<SLOTS_PER_BUCKET; i++){

			if(ht->table[bucket].slots[i].tag == tag){
		...
		bucket = next_bucket(bucket,tag);

		//if(!key.compare("0"))
		//	cout << "bucket2 for 0: " << bucket << endl;
...
void print_ht_table(hashtable_t *ht){
	...
	for(int i=0; i<ht->total_buckets; i++){
		...
		for(int j = 0; j < SLOTS_PER_BUCKET; j++){
			...
			if(ht->table[i].slots[j].ptr){
				printf("   [%u:(%s)]  ",ht->table[i].slots[j].tag,
					ht->table[i].slots[j].ptr->key.c_str());
			}else{
